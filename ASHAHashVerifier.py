...
def right_rotate(value, positions):
...
def compression_function(block, H):
...
def ASHA1(message):
    ...
    for i in range(0, len(message), 64):
        ...
        H = compression_function(block, H)

    return ''.join(f'{x:08x}' for x in H)

def verify_hash():
    given_hash = input("Enter the hash: ")
    test_string = input("Enter the test string: ")
    
    test_hash = ASHA1(test_string.encode('utf-8'))
    
    if test_hash == given_hash:
        print("System unlocked")
    else:
        print("System not unlocked")
